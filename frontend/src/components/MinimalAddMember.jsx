...
export default function MinimalAddMember({ projectId, onMemberAdded }) {
  ...
  const handleSearch = async () => {
    ...
    setLoading(true)
    try {
      const response = await api.get(`/api/projects/users/search?q=${searchQuery}`)
  ...
  const handleAddMembers = async () => {
    ...
    try {
      ...
      setMessage(`Error: ${error.response?.data?.message || 'Failed to add members'}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Card className="w-full max-w-2xl">
      <CardHeader>
        <CardTitle>🎯 Minimal Add Member (Project {projectId})</CardTitle>
      </CardHeader>
